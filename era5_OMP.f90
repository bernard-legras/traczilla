...
! Finds the first date from wfname
   ...
   enddo
   print *,'gribcheck_era5> all index opened'
...
! Allocate temp fields out of the parallel loop
  ...
  !*** else
  allocate(values(NbLon*NbLat,4),vals(NbLon,NbLat,4))
  !*** endif
...
! Temperature
  ...
  call grib_index_select(idx_uvwt(1),'mars.param',130)
  do l=1,NbLev
...
! Zonal wind
  ...
  call grib_index_select(idx_uvwt(2),'mars.param',131)
  do l=1,NbLev
     ...
     call grib_release(igrib_thread)
     vals(:,:,2)=reshape(values(:,2),(/NbLon,NbLat/))  
     do lat=1,NbLat
        uuh(0:NbLon-1,NbLat-lat,Nblev-l+1,n) = vals(:,lat,2)                         
     enddo
...
! Meridional wind
  ...
  call grib_index_select(idx_uvwt(3),'mars.param',132)
  do l=1,NbLev
     ...
     call grib_release(igrib_thread)
     vals(:,:,3)=reshape(values(:,3),(/NbLon,NbLat/))
     do lat=1,NbLat
        vvh(0:NbLon-1,NbLat-lat,Nblev-l+1,n) = vals(:,lat,3)        
     enddo
...
! Surface pressure
  ...
  call grib_index_select(idx_ps,'mars.param',152)
  call grib_index_select(idx_ps,'mars.levelist',1)
  call grib_new_from_index(idx_ps,igrib_thread, iret)
  call grib_get_real4_array(igrib_thread,'values',values(:,4))
  call grib_release(igrib_thread)       
  vals(:,:,4)=reshape(values(:,4),(/Nblon,Nblat/))
  do lat=1,Nblat
      ps(0:NbLon-1,NbLat-lat,1,n) = vals(:,lat,4)                    
  enddo
...
!$OMP END PARALLEL SECTIONS
  ...
  deallocate(values,vals,STAT=err_loc)
...
!$OMP END PARALLEL DO
  ...
  deallocate(lwr,swr,STAT=err_loc)
  if(err_loc/=0) then
