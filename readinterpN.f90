...
! to do: remove useless commented text and variables 

   integer :: ifile, iret, igrib, gotGrid
   !integer :: parCat,parNum,discipl,valSurf
   integer :: gribVer,typSurf,parId,parLev,nbVert
   !integer :: isec1(56),isec2(22+nxmax+nymax)
   integer, allocatable :: inbuff(:)
   real (kind=4) :: zsec2(60+2*nuvzmax)
...
!  get first field   
   ...
   do while (iret/=GRIB_END_OF_FILE)  ! start loop on fields    
      ...
      if (gribVer.eq.1) then ! GRIB Edition 1
        ...
        !elseif ((parCat.eq.2).and.(parNum.eq.8).and.(typSurf.eq.105)) then ! W
        !   isec1(6)=135         ! indicatorOfParameter   
        ...
        !elseif ((parCat.eq.3).and.(parNum.eq.25)) then ! LSP
        !   isec1(6)=142   
...
!********************
   ...
   write(*,'(a,4i5,2L3)')' gridcheck> nuvz_b,nuvz,nwz_b,nwz,u_bot,w_top', &
      nuvz_b,nuvz,nwz_b,nwz,u_bot,w_top
   write(*,*)
...
! PARAMETER akm,bkm DESCRIBE THE HYBRID "ETA" COORDINATE SYSTEM
   ...
   do i=1,nlev_ec+1
        j=i
        k=nlev_ec+1+i
        akm(nlev_ec-i+2)=zsec2(10+j)
        bkm(nlev_ec-i+2)=zsec2(10+k)
   enddo
!     do i=1,nwz
...
! OPENING OF DATA FILE (GRIB CODE)
!
   call grib_open_file(ifile,path(3)(1:len_path(3))//trim(wfname(indj)),'r',iret)
...
!  print *,'readwind> file ',wfname(indj)
   ...
   do while (iret/=GRIB_END_OF_FILE)  ! start loop on fields    
      ...
      if (gribVer.eq.1) then ! GRIB Edition 1
        ...
        !elseif ((parCat.eq.1).and.(parNum.eq.9)) then ! LSP
        !  isec1(6)=142         ! indicatorOfParameter
        !elseif ((parCat.eq.3).and.(parNum.eq.25)) then ! LNSP
        !  isec1(6)=152    
        !elseif ((parCat.eq.1).and.(parNum.eq.10)) then ! CP
        !  isec1(6)=143         ! indicatorOfParameter
        !elseif ((parCat.eq.0).and.(parNum.eq.11).and.(typSurf.eq.1)) then ! SHF
      ...
      ! Skip that part assuming that previous check is enough   
      ...
      k=parLev
      field_identifier: select case (parId)
! 3D fields
      case (130)              !! TEMPERATUR
