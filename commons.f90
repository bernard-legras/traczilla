...
!                         of wind fields
      ...
      logical :: ecmwf_netcdf
      
! ecmwf_netcdf            if true, reads data as netcdf files
...
!*************************************************
! Variables associated with horizontal clipping
!*************************************************

       real(dp) :: xlmin, xlmax, ylmin, ylmax  
       logical setxylim
...
!**************************************
      ...
      logical                ::  diabatic_StratoClim, StratoClimactiv, startfrom0 
      logical                ::  NearRealTime, M10
      integer                ::  NearRealTimeShiftHours 
      integer                ::  fulldate_cloudtop 
      integer (kind=4), allocatable ::  flagTB(:), idxcross(:)
      integer                ::  idx_orgn
      integer (kind=8)       ::  stamp_date
      integer                ::  nb_halfdays_in_batch 

! flagTB              32 bit flag used to code properties for each parcel
! stamp_date          date used to stamp data files in format 110
!                     should be in decimal YYYYMMDDhhmmss format
!                     (hence the need of 64 bits) 
! idx_orgn            first value of the current index		
...
!***************************************
      ...
      integer :: numpart
      integer, allocatable :: itra1(:), itramem(:)
      real(dp), allocatable :: xtra1(:),ytra1(:),ztra1(:)
      integer, allocatable :: itra0(:)
      real(dp), allocatable :: ttra1(:), qtra1(:)
      logical  :: parcel_dyn_alloc
